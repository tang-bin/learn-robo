...
def test_code(test_case):
    ...
    Re = Rz * Ry * Rx
    Rerr = Rz.subs(y, pi) * Ry.subs(p, -pi/2)
    Ree = Re * Rerr
    ...
    Pee = Matrix([[position.x], [position.y], [position.z]])
    ...
    Sa = 1.501 #sqrt(1.5 ** 2 + 0.054 ** 2)
    Sb_x = sqrt(WCx * WCx + WCy * WCy)- 0.35
    Sb_z = WCz - 0.75
    Sb = sqrt(Sb_x * Sb_x + Sb_z * Sb_z)
    Sc = 1.25
    
    angle_e = atan2(Sb_z, Sb_x)
    angle_a = acos((Sc * Sc + Sb*Sb - Sa*Sa) / (2 * Sc * Sb))	    
    theta2 = pi/2 - angle_e - angle_a
    
    angle_b = acos((Sc*Sc + Sa*Sa - Sb*Sb) / (2 * Sa * Sc))    
    theta3 = pi/2 - angle_b - 0.036 #atan2(0.0054, 1.5)
    
    R0_3 = T0_1[:3,:3] * T1_2[0:3,0:3] * T2_3[:3,:3]
    R0_3 = R0_3.evalf(subs={q1: theta1, q2: theta2, q3: theta3})
    R3_6 = R0_3.inv("LU") * Ree
    ...
    theta5 = atan2(sqrt(r02*r02 + r22*r22), r12)
